...
from utils.dataloader_mem import get_im_gt_name_dict, create_dataloaders, RandomHFlip, Resize, LargeScaleJitter
...
class MaskDecoderHQ(MaskDecoder):
    def __init__(self, model_type):
        ...
        self.mem_token = nn.Embedding(1, transformer_dim)
        self.hf_mlp = MLP(transformer_dim, transformer_dim, transformer_dim // 8, 3)
        self.mem_mlp = MLP(transformer_dim, transformer_dim, transformer_dim // 8, 3)
    ...
    def forward(
        ...
        masks_mem = masks[:,slice(self.num_mask_tokens-1, self.num_mask_tokens), :, :]
        
        if hq_token_only:
            return masks_hq, masks_mem
        else:
            return masks_sam, masks_hq, masks_mem
    ...
    def predict_masks(
        ...
        output_tokens = torch.cat([self.iou_token.weight, self.mask_tokens.weight, 
                                   self.hf_token.weight, self.mem_token.weight], dim=0)
        ...
        for i in range(self.num_mask_tokens):
            if i < 4:
                ...
                hyper_in_list.append(self.mem_mlp(mask_tokens_out[:, i, :]))
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, lr_scheduler):
    ...
    for epoch in range(epoch_start,epoch_num): 
        ...
        for data in metric_logger.log_every(train_dataloaders,1000):
            inputs, labels, membrs = data['image'], data['label'], data['membrane']
            if torch.cuda.is_available():
                inputs = inputs.cuda()
                labels = labels.cuda()
                membrs = membrs.cuda()
            ...
            loss_mask_mem, loss_dice_mem = loss_masks(mask_mem, membrs/255.0, len(masks_hq))
